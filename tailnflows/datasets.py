...
from marginal_tail_adaptive_flows.synthetic_experiments.data_generators import (
    copula_generator,
)
...
class ReturnData(Dataset):
    def __init__(
        ...
        x = torch.tensor(joined_data.to_numpy())
        ...
        if data_use == "train":
            self.return_data = x_trn
            self.n = n_trn
        elif data_use == "test":
            self.return_data = x_tst
            self.n = n_tst
        elif data_use == "validate":
            self.return_data = x_val
            self.n = n_val
        else:
